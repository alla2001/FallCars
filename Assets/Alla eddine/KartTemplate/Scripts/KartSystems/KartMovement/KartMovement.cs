...
namespace KartGame.KartSystems
{
    ...
    public class KartMovement : MonoBehaviour, IKartCollider, IMovable, IKartInfo
    {
        ...
        public KartPowerManager powerManager;
        ...
        private Vector3 m_RigidbodyPosition;
        private Vector3 m_Velocity;
        private Vector3 m_Movement;
        private bool m_IsGrounded;
        private GroundInfo m_CurrentGroundInfo;
        private Rigidbody m_Rigidbody;
        private CapsuleCollider m_Capsule;
        private IRacer m_Racer;
        private List<IKartModifier> m_CurrentModifiers = new List<IKartModifier>(16); // The karts stats are based on a list of modifiers.  Each affects the results of the previous until the modified stats are calculated.
        private List<IKartModifier> m_TempModifiers = new List<IKartModifier>(8);
        private KartStats m_ModifiedStats; // The stats that are used to calculate the kart's velocity.
        private RaycastHit[] m_RaycastHitBuffer = new RaycastHit[8];
        private Collider[] m_ColliderBuffer = new Collider[8];
        private Quaternion m_DriftOffset = Quaternion.identity;
        ...
        private Vector3 m_RepositionPositionDelta;
        private Quaternion m_RepositionRotationDelta = Quaternion.identity;

        private const int k_MaxPenetrationSolves = 3;
        private const float k_GroundToCapsuleOffsetDistance = 0.025f;
        private const float k_Deadzone = 0.01f;
        private const float k_VelocityNormalAirborneDot = 0.5f;
        ...
        /// </summary>
        private bool GetNearestFromRaycast(Ray ray, float rayDistance, int layerMask, QueryTriggerInteraction query, out RaycastHit hit)
        {
        ...
        /// </summary>
        private void GroundNormal(float deltaTime, ref Quaternion rotationStream, GroundInfo currentGroundInfo, GroundInfo nextGroundInfo)
        {
        ...
        private void CalculateDrivingVelocity(float deltaTime, GroundInfo groundInfo, Quaternion rotationStream)
        {
            ...
            if (groundInfo.isGrounded)
            {
                ...
                if (acceleration > -k_Deadzone && acceleration < k_Deadzone)    // No acceleration input.
                    ...
                    localVelocity.z = Mathf.MoveTowards(localVelocity.z, m_ModifiedStats.topSpeed * powerManager.power, acceleration * powerManager.power * m_ModifiedStats.acceleration * deltaTime);
                else if (localVelocity.z > k_Deadzone)                         // Negative acceleration input and going forwards.
                    localVelocity.z = Mathf.MoveTowards(localVelocity.z, 0f, -acceleration * powerManager.power * m_ModifiedStats.braking * deltaTime);
                else                                                           // Negative acceleration input and not going forwards.
                    localVelocity.z = Mathf.MoveTowards(localVelocity.z, -m_ModifiedStats.reverseSpeed * powerManager.power, -acceleration * m_ModifiedStats.reverseAcceleration * deltaTime);
            }
            localVelocity += force;
            force = Vector3.zero;
        ...
        /// <param name="kartModifier"></param>
        public void ClearKartModifier()
        {
            m_CurrentModifiers.Clear();
        }
